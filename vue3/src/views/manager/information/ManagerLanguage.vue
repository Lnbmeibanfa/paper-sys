...
const loadLanguageData = () => {
  selectLanguageDataAPI(pageNum.value, pageSize.value, name.value).then((res) => {
    if (res.code === '200') {
      data.languageData = res.data?.list || []
...
const insert = () => {
  addLanguageAPI(data.formData).then((res) => {
    if (res.code === '200') {
...
const update = () => {
  ...
  updateLanguageAPI(data.formData).then((res) => {
    if (res.code === '200') {
...
<template>
  <div class="language-manage">
    <div class="search-box card">
      ...
      <el-button type="primary" size="default" @click="loadLanguageData">查询</el-button>
  ...
  <div class="display card">
    ...
    >
      ...
      <el-table-column prop="name" label="论文编程语言" />
  ...
  <div class="pagination card">
    <el-pagination
      ...
      @current-change="loadLanguageData"
    />
  </div>
  <!--dialog-->
  <el-dialog v-model="data.dialogVisible" title="增加编程语言" width="700">
    <el-form label-width="70px" :model="data.formData">
      <el-form-item label="编程语言">
        <el-input v-model="data.formData.name" placeholder="请输入编程语言" clearable></el-input>
      </el-form-item>
    </el-form>
    <template #footer>
